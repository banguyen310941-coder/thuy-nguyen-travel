...
export default async function ProductPage({searchParams}:Props){
 ...
 if(slug){const query=new URLSearchParams();for(const [key,value] of Object.entries(raw)){if(key==='slug'||value===undefined)continue;if(Array.isArray(value))value.forEach(item=>query.append(key,item));else query.set(key,value)}const suffix=query.toString()?`?${query.toString()}`:'';permanentRedirect(`/product/${encodeURIComponent(slug)}${suffix}`)}
