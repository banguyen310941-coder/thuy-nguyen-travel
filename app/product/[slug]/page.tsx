...
export async function generateMetadata({params}:Props):Promise<Metadata>{
 const {slug}=await params;const product=await getPublishedProductSeo(slug);const base=getSiteUrl();const canonical=product?.type==='Du thuyền'?`${base}/du-thuyen/${encodeURIComponent(slug)}`:product?.type.includes('Tour')?`${base}/tour-du-lich/${encodeURIComponent(slug)}`:`${base}/san-pham/${encodeURIComponent(slug)}`;
...
export default async function CanonicalProductPage({params}:Props){
 ...
 if(product?.type==='Du thuyền'||initialProduct?.type==='Du thuyền')permanentRedirect(`/du-thuyen/${encodeURIComponent(slug)}`);
 if(product?.type.includes('Tour')||initialProduct?.type?.includes('Tour'))permanentRedirect(`/tour-du-lich/${encodeURIComponent(slug)}`);
