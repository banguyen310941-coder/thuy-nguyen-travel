...
export async function generateMetadata({params}:Props):Promise<Metadata>{
 const {slug}=await params;const product=await getPublishedProductSeo(slug);const base=getSiteUrl();const canonical=`${base}/product/${encodeURIComponent(slug)}`;
...
export default async function CanonicalProductPage({params}:Props){
 const {slug}=await params;const product=await getPublishedProductSeo(slug);const base=getSiteUrl();const url=`${base}/product/${encodeURIComponent(slug)}`;
