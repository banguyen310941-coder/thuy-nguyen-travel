...
import {
  ...
  AdminSeoPanel,
  AdminSettingsPanel,
} from "@/components/AdminUtilityPanels";
...
const permission: Record<string, string> = {
  "Điều hành Sale": "customers",
  "Điều hành dịch vụ": "bookings",
  ...
  "Khách sạn": "stays",
  "Du thuyền": "cruises",
  "Bài viết / Cẩm nang": "content",
  "Media (Ảnh/Video)": "media",
  "Giao diện (Trang chủ)": "settings",
  "Cấu hình SEO": "settings",
...
export default function AdminPage() {
  const [active, setActive] = useState("Tổng quan"),
    [current, setCurrent] = useState<AdminStaff | null>(null),
  ...
  useEffect(() => {
    const load = () => {
      setCurrent(readAdminSession() ? readCurrentStaff() : null);
      setReady(true);
    };
    load();
    window.addEventListener("happygo-admin-auth", load);
    window.addEventListener("tn-staff-updated", load);
    return () => {
      window.removeEventListener("happygo-admin-auth", load);
      window.removeEventListener("tn-staff-updated", load);
    };
  }, []);
  if (!ready)
    return (
      ...
      <main className="admin-login-page">
        <form
          ...
          onSubmit={(e) => {
            ...
            setMsg(r.message);
            if (r.ok && r.staff) setCurrent(r.staff);
          }}
        >
          <div className="admin-login-logo">HG</div>
          <small>HAPPYGO TRAVEL · QUẢN TRỊ</small>
          ...
          {msg && <div className="admin-login-message">{msg}</div>}
          <button className="admin-primary">Đăng nhập</button>
  ...
  if (!allowed(active)) setTimeout(() => setActive("Tổng quan"), 0);
  const primary = mobilePrimary
      .map((n) => visible.find(([, x]) => x === n))
      .filter(Boolean) as (typeof modules)[number][],
    more = visible.filter(([, n]) => !mobilePrimary.includes(n));
  return (
    <div className="admin-app">
      <AdminProductMigration />
      <aside className="admin-sidebar">
        <div className="admin-brand">
          <span>HG</span>
          <div>
            <b>HAPPYGO</b>
            <small>TRAVEL ADMIN</small>
          </div>
        </div>
        <nav>
          {visible.map(([i, n]) => (
      ...
      <nav className="admin-mobile-nav">
        {primary.map(([i, n]) => (
          <button
            key={n}
            className={active === n && !mobileMore ? "active" : ""}
            onClick={() => go(n)}
          >
            <i>{i}</i>
            <span>
        ...
        <button
          className={
            mobileMore || !mobilePrimary.includes(active) ? "active" : ""
          }
        ...
        >
          ...
          >
            <div className="admin-mobile-more-head">
              ...
              <button onClick={() => setMobileMore(false)}>×</button>
            </div>
            ...
            <div className="admin-mobile-account">
              <div>
                <b>{current.name}</b>
                <small>
                  {owner ? "Chủ tài khoản" : current.department || current.role}
                </small>
              </div>
              <button
                onClick={() => {
      ...
      <main className="admin-main">
        ...
        {active === "Tổng quan" && (
          <>
            <div className="admin-welcome">
              <div>
                <small>TỔNG QUAN ĐIỀU HÀNH</small>
                <h2>
                  {owner
                    ? "Trung tâm điều hành HappyGo Travel"
                    : `Xin chào ${current.name}`}
                </h2>
                <p>
                  Thông báo và công việc cần xử lý được tập trung theo đúng
                  quyền nghiệp vụ của tài khoản.
                </p>
              </div>
            </div>
            <AdminTodayWork
            ...
            />
            {allowed("Đơn đặt dịch vụ") && (
              <>
                <AdminSalesDashboard
                  openBookings={() => go("Đơn đặt dịch vụ")}
                  openCustomers={() => go("Khách hàng / CRM")}
                />
            ...
            {allowed("Điều hành Sale") && (
              <AdminCrmWorkboard
                openBookings={() => go("Đơn đặt dịch vụ")}
                openCustomers={() => go("Khách hàng / CRM")}
              />
            )}{" "}
            {owner && <AdminConnectivityPanel />}
          </>
        )}
        {active === "Điều hành Sale" && allowed(active) && (
          <>
            <AdminCrmWorkboard
              openBookings={() => go("Đơn đặt dịch vụ")}
              openCustomers={() => go("Khách hàng / CRM")}
            />
            <AdminCrmPipeline openCustomers={() => go("Khách hàng / CRM")} />
            <AdminCustomerRetention />
          </>
        )}
        {active === "Điều hành dịch vụ" && allowed(active) && (
          <>
            <AdminServiceOperations />
            <AdminSupplierOrders />
          </>
        )}
        {active === "Phiếu thu khách" && allowed(active) && (
          <AdminCustomerReceipts />
        )}
        {active === "Duyệt & thanh toán" && allowed(active) && (
        ...
        {active === "Chat nội bộ" && <AdminTeamChat />}
        {active === "Khách hàng / CRM" && allowed(active) && (
          <>
            <AdminCustomers />
            <AdminCustomerRetention />
        ...
        {active === "Quản lý đối tác" && allowed(active) && (
          <AdminPartnerManager />
        )}
        {active === "Hỗ trợ đối tác" && allowed(active) && (
          <PartnerSupportCenter />
        )}
        {active === "Đơn đặt dịch vụ" && allowed(active) && (
          <>
            <AdminSupplierOrders />
            <AdminBookingOperations openBookings={() => {}} />
            <AdminBookings />
          </>
        )}
        {active === "Voucher khách hàng" && allowed(active) && (
          <AdminCustomerVouchers />
        )}
        {active === "Nhân viên & phân quyền" && allowed(active) && (
          <AdminStaffManager />
        )}
        ...
        {active === "Bài viết / Cẩm nang" && allowed(active) && (
          <AdminContentEditor />
        )}
        {active === "Tour du lịch" && allowed(active) && (
          <AdminTourEditorSimple />
        )}
        {active === "Sản phẩm" && allowed(active) && (
          <AdminProductManagerV3 type="Sản phẩm" />
        )}
        {(["Villa & Resort", "Khách sạn", "Du thuyền"] as const).includes(
          active as never,
        ) &&
          allowed(active) && (
            <AdminProductManagerV3
              type={active as "Villa & Resort" | "Khách sạn" | "Du thuyền"}
            />
          )}{" "}
        {active === "Giao diện (Trang chủ)" && allowed(active) && (
          <AdminHomepageEditor />
        )}
        {active === "Thùng rác / Xóa dữ liệu" && owner && (
          <AdminDeleteManager />
        )}
        {active === "Media (Ảnh/Video)" && allowed(active) && (
          <AdminMediaManager />
        )}
