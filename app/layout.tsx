...
export async function generateMetadata():Promise<Metadata>{
 ...
 return {
  ...
  twitter:{card:'summary_large_image',title,description,...(image?{images:[image]}:{})},other:{'mobile-web-app-capable':'yes','x-ui-version':'happygo-public-stars-20260906-prod'}
 };
}
...
export default async function RootLayout({children}:Readonly<{children:React.ReactNode}>){
 ...
 return <html lang="vi"><body id="top" data-ui-version="happygo-public-stars-20260906-prod"><script type="application/ld+json">{JSON.stringify(organization)}</script><MarketingAttributionCapture/><PwaRegister/><SiteChrome>{children}</SiteChrome></body></html>;
}