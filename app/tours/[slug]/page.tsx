...
export default async function TourDetailPage({params}:{params:Promise<{slug:string}>}){
 ...
 const t=tours.find(x=>x.slug===slug);
 if(t){
  const url=`${base}/tour-du-lich/${t.slug}`;const price=amount(t.priceFrom);
  const trip:any={"@context":"https://schema.org","@type":"TouristTrip","name":t.name,"description":t.summary,"image":[t.image,...(t.gallery||[])],"url":url,"touristType":"Du khách Việt Nam","provider":{"@type":"TravelAgency","name":"HappyGo Travel","url":base,"telephone":"+84969973949"}};
  if(price)trip.offers={"@type":"Offer","priceCurrency":"VND","price":price,"url":url,"availability":"https://schema.org/InStock"};
  if(t.rating&&t.reviewCount)trip.aggregateRating={"@type":"AggregateRating","ratingValue":t.rating,"reviewCount":t.reviewCount,"bestRating":10,"worstRating":1};
  const schema:any[]=[trip,{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Trang chủ","item":base},{"@type":"ListItem","position":2,"name":"Tour du lịch","item":`${base}/tour-du-lich`},{"@type":"ListItem","position":3,"name":t.name,"item":url}]}];
  if(t.faq?.length)schema.push({"@context":"https://schema.org","@type":"FAQPage","mainEntity":t.faq.map(x=>({"@type":"Question","name":x.q,"acceptedAnswer":{"@type":"Answer","text":x.a}}))});
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/><TourDetailClient base={t}/></>
 }
 const cms=await getPublishedTourSeo(slug);
 if(!cms)notFound();
 ...
 return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/><CmsTourDetail slug={slug} initialTour={initialTour}/></>
}
