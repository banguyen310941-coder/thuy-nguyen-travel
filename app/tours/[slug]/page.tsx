...
export default async function TourDetailPage({params}:{params:Promise<{slug:string}>}){
 ...
 const gallery=tour.gallery?.length?tour.gallery:[tour.image,tour.image,tour.image];
 ...
 return <div className="tour-rich-page">
  ...
  <section className="container tour-content-layout"><main>
   ...
   <section id="price" className="tour-content-card"><h2>Bảng giá & dịch vụ</h2><div className="tour-price-table"><div><b>Người lớn</b><strong>{tour.priceFrom||'Liên hệ'}</strong><span>/ khách</span></div><div><b>Trẻ em</b><strong>Liên hệ</strong><span>Theo độ tuổi</span></div><div><b>Phụ thu phòng đơn</b><strong>Liên hệ</strong><span>Theo lịch</span></div></div><div className="tour-two-col"><div><h3>Giá bao gồm</h3><ul>{(tour.included||['Dịch vụ theo chương trình']).map(x=><li key={x}>✓ {x}</li>)}</ul></div><div><h3>Không bao gồm</h3><ul>{(tour.excluded||['Chi phí cá nhân']).map(x=><li key={x}>– {x}</li>)}</ul></div></div></section>
   <section className="tour-content-card"><h2>Chính sách</h2><ul>{(tour.policies||['Chính sách đặt cọc và hoàn hủy được xác nhận theo từng lịch khởi hành.']).map(x=><li key={x}>{x}</li>)}</ul></section>
