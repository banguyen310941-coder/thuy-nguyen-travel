...
export async function GET(){
 ...
 try{
  const sql=db();const [rows,productionProducts]=await Promise.all([sql`select distinct on (entity_id) entity_id,after_data,created_at from audit_logs where entity_type='admin_shared_state' order by entity_id,created_at desc,id desc`,relationalProducts(sql)]);
