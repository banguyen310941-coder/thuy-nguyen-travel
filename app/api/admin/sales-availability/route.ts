...
export async function GET(req:NextRequest){
 if(!hasDatabase())return NextResponse.json({error:'Database chưa sẵn sàng.'},{status:503});const actor=await adminActor(req);if(!actor)return NextResponse.json({error:'Unauthorized'},{status:401});if(!isSales(actor)&&!elevated(actor))return NextResponse.json({error:'Tài khoản không thuộc nhóm Sale.'},{status:403});
 try{const current=await states();return NextResponse.json({ok:true,...current,actorId:actor.id,canToggle:true,canManage:elevated(actor)},{headers:{'Cache-Control':'no-store, max-age=0'}})}catch(error){console.error('sales_availability_get_failed',error);return NextResponse.json({error:'Không đọc được trạng thái nhận khách.'},{status:500})}
}
