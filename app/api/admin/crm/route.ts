...
export async function POST(req:NextRequest){
 ...
 try{
  if(action==='create'){
   ...
   const duplicate=phone?await sql`select id from customers where regexp_replace(coalesce(phone,''),'\\D','','g')=regexp_replace(${phone},'\\D','','g') limit 1`:await sql`select id from customers where lower(coalesce(email,''))=${email} limit 1`;
