...
async function availableSales(sql:any){const list=await sales(sql);if(!list.length)return[];const rows=await sql`select distinct on (entity_id) entity_id,after_data from audit_logs where entity_type='sales_availability' order by entity_id,created_at desc,id desc`;const state=new Map(rows.map((row:any)=>[String(row.entity_id),row.after_data as any]));return list.filter((sale:any)=>{const value:any=state.get(String(sale.id));return value?.receivingCustomers!==false})}
...
async function nextSale(sql:any){const list=await availableSales(sql);if(!list.length)return null;const state=await rotation(sql);const last=String(state?.last_staff_id||'');const index=list.findIndex((s:any)=>String(s.id)===last);return list[(index+1+list.length)%list.length]||list[0]}
...
export async function GET(req:NextRequest){
 ...
 try{
  ...
  return NextResponse.json({ok:true,customers:base.map(shapeCustomer),sales:saleRows.map((s:any)=>({id:String(s.id),name:String(s.name),email:String(s.email||''),receivingCustomers:available.has(String(s.id))})),rotation:{enabled:Boolean(rotationState?.enabled),lastStaffId:rotationState?.last_staff_id?String(rotationState.last_staff_id):'',assignedCount:Number(rotationState?.assigned_count||0)},capabilities:{manage:elevated(actor.role),create:true}})
...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  if(action==='distribute'){
   if(!elevated(actor.role))return NextResponse.json({error:'Chỉ Quản trị được chia khách.'},{status:403});const state=await rotation(sql);if(!state.enabled)return NextResponse.json({error:'Hãy bật chia khách tự động trước.'},{status:400});const active=await availableSales(sql);if(!active.length)return NextResponse.json({error:'Không có Sale nào đang bật nhận khách mới.'},{status:409});const pending=await sql`select c.id from customers c left join customer_assignments ca on ca.customer_id=c.id where ca.customer_id is null order by c.created_at asc limit 500`;let count=0;for(const row of pending){if(await assignRoundRobin(sql,String(row.id)))count++}return NextResponse.json({ok:true,count});
  }
  if(action==='status'){
