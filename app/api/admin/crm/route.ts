...
export async function POST(req:NextRequest){
 ...
 try{
  if(action==='create'){
   ...
   const rows=await sql`insert into customers(name,phone,email,status,source,note,updated_at) values(${name},${phone||null},${email||null},'lead',${source||null},${note||null},now()) returning *`;const customer=rows[0];const leadKind=salesLeadKind(body.leadKind);
