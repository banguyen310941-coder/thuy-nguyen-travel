...
async function availableSales(sql:any,leadKind:SalesLeadKind='other'){const list=await sales(sql);if(!list.length)return[];const rows=await sql`select distinct on (entity_id) entity_id,after_data from audit_logs where entity_type='sales_availability' order by entity_id,created_at desc,id desc`;const state=new Map(rows.map((row:any)=>[String(row.entity_id),row.after_data as any]));const routed=filterSalesForLead(list,leadKind);return routed.filter((sale:any)=>{const value:any=state.get(String(sale.id));return value?.receivingCustomers!==false})}
async function rotation(sql:any,id=1){await sql`insert into sales_rotation(id,enabled,assigned_count) values(${id},false,0) on conflict(id) do nothing`;const rows=await sql`select id,last_staff_id,enabled,assigned_count,updated_at from sales_rotation where id=${id}`;return rows[0]}
function rotationId(leadKind:SalesLeadKind){return leadKind==='tour'?2:leadKind==='stay'?3:1}
async function nextSale(sql:any,leadKind:SalesLeadKind){const list=await availableSales(sql,leadKind);if(!list.length)return null;const state=await rotation(sql,rotationId(leadKind));const last=String(state?.last_staff_id||'');const index=list.findIndex((s:any)=>String(s.id)===last);return list[(index+1+list.length)%list.length]||list[0]}
async function assignRoundRobin(sql:any,customerId:string,leadKind:SalesLeadKind='other'){const existing=await sql`select customer_id from customer_assignments where customer_id=${customerId} limit 1`;if(existing.length)return null;const master=await rotation(sql,1);if(!master?.enabled)return null;const sale=await nextSale(sql,leadKind);if(!sale)return null;const rid=rotationId(leadKind);await sql`insert into customer_assignments(customer_id,staff_id,source,assigned_at) values(${customerId},${String(sale.id)},'round_robin',now()) on conflict(customer_id) do nothing`;await sql`update sales_rotation set last_staff_id=${String(sale.id)},assigned_count=assigned_count+1,updated_at=now() where id=${rid}`;if(rid!==1)await sql`update sales_rotation set assigned_count=assigned_count+1,updated_at=now() where id=1`;return sale}
...
export async function GET(req:NextRequest){
 ...
 try{
  const base=elevated(actor.role)?await sql`
   ...
   group by c.id,ca.staff_id,ca.source,ca.assigned_at,s.name order by c.updated_at desc limit 1000`;
  const rotationState=await rotation(sql);const saleRows=elevated(actor.role)?await sales(sql):[];const available=elevated(actor.role)?new Set((await availableSales(sql)).map((s:any)=>String(s.id))):new Set<string>();
  return NextResponse.json({ok:true,customers:base.map(shapeCustomer),sales:saleRows.map((s:any)=>({id:String(s.id),name:String(s.name),email:String(s.email||''),receivingCustomers:available.has(String(s.id)),receiveTourLeads:Array.isArray(s.permissions)&&s.permissions.includes(TOUR_LEAD_PERMISSION),receiveStayLeads:Array.isArray(s.permissions)&&s.permissions.includes(STAY_LEAD_PERMISSION)})),rotation:{enabled:Boolean(rotationState?.enabled),lastStaffId:rotationState?.last_staff_id?String(rotationState.last_staff_id):'',assignedCount:Number(rotationState?.assigned_count||0)},capabilities:{manage:elevated(actor.role),create:true}})
...
export async function POST(req:NextRequest){
 ...
 try{
  if(action==='create'){
   ...
   const rows=await sql`insert into customers(name,phone,email,status,source,note,updated_at) values(${name},${phone||null},${email||null},'lead',${source||null},${note||null},now()) returning *`;const customer=rows[0];const leadKind=salesLeadKind(body.leadKind),saleRows=await sales(sql),eligibleSales=filterSalesForLead(saleRows,leadKind);
   const requested=String(body.salesStaffId||'');if(requested){if(!uuid.test(requested))return NextResponse.json({error:'Nhân viên Sale không hợp lệ.'},{status:400});if(!elevated(actor.role)&&requested!==actor.id)return NextResponse.json({error:'Bạn không có quyền giao khách cho Sale khác.'},{status:403});const staff=saleRows.find((item:any)=>String(item.id)===requested);if(!staff)return NextResponse.json({error:'Không tìm thấy Sale đang hoạt động.'},{status:400});if(!eligibleSales.some((item:any)=>String(item.id)===requested))return NextResponse.json({error:leadKind==='tour'?'Sale này chưa được bật quyền nhận khách Tour.':leadKind==='stay'?'Sale này chưa được bật quyền nhận khách Villa / Khách sạn.':'Sale này không thuộc nhóm nhận khách.'},{status:400});await sql`insert into customer_assignments(customer_id,staff_id,source) values(${String(customer.id)},${requested},'manual') on conflict(customer_id) do update set staff_id=excluded.staff_id,source='manual',assigned_at=now()`}
   else if(actor.role==='sales'){if(!eligibleSales.some((item:any)=>String(item.id)===actor.id))return NextResponse.json({error:leadKind==='tour'?'Bạn chưa được phân quyền nhận khách Tour.':leadKind==='stay'?'Bạn chưa được phân quyền nhận khách Villa / Khách sạn.':'Bạn chưa được phân quyền nhận nhóm khách này.'},{status:403});await sql`insert into customer_assignments(customer_id,staff_id,source) values(${String(customer.id)},${actor.id},'manual') on conflict(customer_id) do nothing`}
  ...
  if(action==='distribute'){
   if(!elevated(actor.role))return NextResponse.json({error:'Chỉ Quản trị được chia khách.'},{status:403});const state=await rotation(sql,1);if(!state.enabled)return NextResponse.json({error:'Hãy bật chia khách tự động trước.'},{status:400});const active=await availableSales(sql);if(!active.length)return NextResponse.json({error:'Không có Sale nào đang bật nhận khách mới.'},{status:409});const pending=await sql`select c.id,coalesce((select bi.data_snapshot->>'leadKind' from bookings b join booking_items bi on bi.booking_id=b.id where b.customer_id=c.id order by b.created_at desc,bi.id limit 1),(select bi.data_snapshot->>'kind' from bookings b join booking_items bi on bi.booking_id=b.id where b.customer_id=c.id order by b.created_at desc,bi.id limit 1),'') as lead_kind from customers c left join customer_assignments ca on ca.customer_id=c.id where ca.customer_id is null order by c.created_at asc limit 500`;let count=0;for(const row of pending){if(await assignRoundRobin(sql,String(row.id),salesLeadKind(row.lead_kind)))count++}return NextResponse.json({ok:true,count});
  }
  if(action==='status'){
