...
async function customerAccess(actor:AdminActor,customerId:string){if(!uuid.test(customerId))return null;const sql=db();if(elevated(actor)){const rows=await sql`select c.id,c.name,c.phone,c.email from customers c where c.id=${customerId} limit 1`;return rows[0]||null}const rows=await sql`select c.id,c.name,c.phone,c.email from customers c join customer_assignments ca on ca.customer_id=c.id where c.id=${customerId} and ca.staff_id=${actor.id} limit 1`;return rows[0]||null}
