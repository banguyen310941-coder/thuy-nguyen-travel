...
export async function POST(req:NextRequest){if(!hasDatabase())return NextResponse.json({error:'Database chưa sẵn sàng.'},{status:503});if(requestBodyTooLarge(req,4_194_304))return NextResponse.json({error:'Dữ liệu trang chủ quá lớn.'},{status:413});const actor=await adminActor(req,'settings');if(!actor)return NextResponse.json({error:'Unauthorized'},{status:401});const parsed=await readBoundedJson(req,4_194_304);if(parsed.tooLarge)return NextResponse.json({error:'Dữ liệu trang chủ quá lớn.'},{status:413});const body=parsed.body as any,value=body&&typeof body.value==='object'&&!Array.isArray(body.value)?body.value:null;if(!value)return NextResponse.json({error:'Nội dung trang chủ không hợp lệ.'},{status:400});try{const before=await current(),updatedAt=new Date().toISOString(),envelope={value,updatedAt,updatedBy:actor.name};await db()`insert into audit_logs(actor_staff_id,action,entity_type,entity_id,before_data,after_data) values(${actor.id},'homepage.save',${ENTITY},${KEY},${JSON.stringify(before.value)}::jsonb,${JSON.stringify(envelope)}::jsonb)`;return NextResponse.json({ok:true,value,updatedAt})}catch(error){console.error('homepage_cms_save_failed',error);return NextResponse.json({error:'Không thể lưu trang chủ production.'},{status:500})}}
