...
export async function POST(req:NextRequest){
 if(!hasDatabase())return NextResponse.json({error:'Database chưa sẵn sàng.'},{status:503});const actor=await adminActor(req,'bookings');if(!actor)return NextResponse.json({error:'Unauthorized'},{status:401});if(!operations(actor))return NextResponse.json({error:'Chỉ Điều hành/Quản trị được cập nhật voucher.'},{status:403});if(requestBodyTooLarge(req,65_536))return NextResponse.json({error:'Dữ liệu voucher quá lớn.'},{status:413});const parsed=await readBoundedJson(req,65_536);if(parsed.tooLarge)return NextResponse.json({error:'Dữ liệu voucher quá lớn.'},{status:413});const body=parsed.body;const action=String(body.action||'');
