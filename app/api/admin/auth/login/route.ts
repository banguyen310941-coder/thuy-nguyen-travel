...
export async function POST(req:NextRequest){
  ...
  const sql=db();
  try{const rows=await sql`select id,name,email,phone,password_hash,role,department,status,permissions,created_at from staff where lower(email)=lower(${email}) limit 1`;const row=rows[0];if(!row||row.status!=='active'||!verifyPassword(password,String(row.password_hash||'')))return NextResponse.json({error:'Email hoặc mật khẩu không đúng.'},{status:401});if(row.role==='affiliate')return NextResponse.json({error:'Tài khoản CTV đăng nhập tại /affiliate.'},{status:403});const response=NextResponse.json({ok:true,staff:shape(row)});setSessionCookie(response,COOKIE,'admin',String(row.id));return response}catch(error){console.error('admin_login_failed',error);return NextResponse.json({error:'Không thể đăng nhập quản trị lúc này.'},{status:500})}
}
