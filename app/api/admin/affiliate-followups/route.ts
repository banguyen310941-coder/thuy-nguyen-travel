...
export async function GET(req:NextRequest){
 ...
 try{
  ...
  const affiliates=all
   ?await sql`select a.id,s.name,s.email,a.phone,a.zalo,a.referral_code,a.status,a.sales_owner_id,owner.name as sales_owner_name from affiliates a join staff s on s.id=a.user_id left join staff owner on owner.id=a.sales_owner_id order by s.name`
   :await sql`select a.id,s.name,s.email,a.phone,a.zalo,a.referral_code,a.status,a.sales_owner_id,owner.name as sales_owner_name from affiliates a join staff s on s.id=a.user_id left join staff owner on owner.id=a.sales_owner_id where a.sales_owner_id=${actor.id} order by s.name`;
