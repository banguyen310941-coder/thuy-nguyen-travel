...
function esc(v:unknown){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#39;'}[c]||c))}
...
export async function POST(req:NextRequest){
 ...
 const body=await req.json().catch(()=>({}));let name=String(body.customerName||'').trim(),phone=normalizePhone(body.phone),email=String(body.email||'').trim().toLowerCase();const product=String(body.product||'Dịch vụ HappyGo').trim();
 ...
 try{
  ...
  return NextResponse.json({ok:true,code:bookingCode,assignedSales:assignment?.name||null,accountLinked:Boolean(session)});
