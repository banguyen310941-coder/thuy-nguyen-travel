...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  await sql`with new_booking as (insert into bookings(code,customer_id,status,source,start_date,end_date,adults,children,rooms,customer_name_snapshot,phone_snapshot,email_snapshot,note,selling_total_vnd) values(${bookingCode},${customerId},'new',${String(body.source||'website')},${body.startDate||null},${body.endDate||null},${Math.max(1,Number(body.adults)||1)},${Math.max(0,Number(body.children)||0)},${Math.max(1,Number(body.rooms)||1)},${name},${phone},${email||null},${String(body.note||'')},0) returning id) insert into booking_items(booking_id,product_name_snapshot,quantity,selling_price_vnd,data_snapshot) select id,${product},1,0,${JSON.stringify({kind:String(body.kind||'Dịch vụ')})}::jsonb from new_booking`;
