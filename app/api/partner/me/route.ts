...
async function current(req:NextRequest){
  ...
  const sql=db();
  const rows=await sql`select p.id,p.name,p.email,p.phone,p.status,p.commission_percent,p.created_at,a.contact_name,a.website,a.tax_code,a.address,a.data from partners p join partner_accounts a on a.partner_id=p.id where p.id=${session.id} limit 1`;
  return rows[0]||null;
}
...
export async function GET(req:NextRequest){
  ...
  try{const row=await current(req);if(!row)return NextResponse.json({error:'Unauthorized'},{status:401});return NextResponse.json({ok:true,partner:shape(row)})}catch(error){console.error('partner_me_get_failed',error);return NextResponse.json({error:'Không đọc được hồ sơ đối tác.'},{status:500})}
}
...
export async function PATCH(req:NextRequest){
  ...
  const sql=db();
  try{
    await sql`update partners set name=${name},phone=${phone},updated_at=now() where id=${session.id}`;
    await sql`update partner_accounts set contact_name=${contact},website=${website||null},tax_code=${taxCode||null},address=${address||null},updated_at=now() where partner_id=${session.id}`;
    const row=await current(req);return NextResponse.json({ok:true,partner:shape(row)});
