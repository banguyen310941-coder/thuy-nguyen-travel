...
export async function GET(req:NextRequest){
  ...
  try{return NextResponse.json({ok:true,tickets:await ticketRows(session.id)})}catch(error){console.error('partner_support_get_failed',error);return NextResponse.json({error:'Không đọc được yêu cầu hỗ trợ.'},{status:500})}
}
...
export async function POST(req:NextRequest){
  ...
  const sql=db();
  try{
    const partnerRows=await sql`select p.name,a.contact_name from partners p left join partner_accounts a on a.partner_id=p.id where p.id=${session.id}`;
    const sender=String(partnerRows[0]?.contact_name||partnerRows[0]?.name||'Đối tác HappyGo');
    if(action==='reply'){
      const ticketId=String(body.ticketId||'');const rows=await sql`select id,status from partner_support_tickets where id=${ticketId} and partner_id=${session.id} limit 1`;if(!rows.length)return NextResponse.json({error:'Không tìm thấy yêu cầu.'},{status:404});
      ...
      const rows=await sql`insert into partner_support_tickets(partner_id,subject,category,status) values(${session.id},${subject},${category},'new') returning id`;
    ...
    return NextResponse.json({ok:true,tickets:await ticketRows(session.id)});
