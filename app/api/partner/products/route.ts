...
export async function POST(req:NextRequest){
  ...
  try{
    ...
    if(/^[0-9a-f-]{36}$/i.test(id)){
      rows=await sql`update products set slug=${slug},type=${type},name=${name},status=${status},description=${String(product.summary||'')||null},retail_price_vnd=${retail},net_price_vnd=${net||null},promo_price_vnd=${promo||null},data=${JSON.stringify(data)}::jsonb,updated_at=now() where id=${id} and partner_id=${actor.id} returning *`;
      if(!rows.length)return NextResponse.json({error:'Không tìm thấy sản phẩm.'},{status:404});
    }else{
      rows=await sql`insert into products(partner_id,slug,type,name,status,description,retail_price_vnd,net_price_vnd,promo_price_vnd,data) values(${actor.id},${slug},${type},${name},${status},${String(product.summary||'')||null},${retail},${net||null},${promo||null},${JSON.stringify(data)}::jsonb) returning *`;
    }
    const partner=await sql`select name from partners where id=${actor.id}`;
