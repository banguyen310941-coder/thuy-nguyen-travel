...
export async function GET(req:NextRequest){
  ...
  const sql=db();
  try{
    const rows=await sql`select id,data from products where partner_id=${actor.id}`;
...
export async function PUT(req:NextRequest){
  ...
  try{
    for(const [productId,list] of grouped){
      ...
      await sql`update products set data=data||${payload}::jsonb,net_price_vnd=${money(base.agencyPrice)||null},retail_price_vnd=${money(base.retailPrice)},promo_price_vnd=${money(base.promoPrice)||null},updated_at=now() where id=${productId} and partner_id=${actor.id}`;
    }
