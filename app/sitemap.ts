...
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
 ...
 const products=production.filter(item=>!item.type.includes('Tour')).map(item=>({url:`${base}/${item.type==='Du thuyền'?'du-thuyen':'san-pham'}/${encodeURIComponent(item.slug)}`,...(item.updatedAt&&Number.isFinite(+new Date(item.updatedAt))?{lastModified:new Date(item.updatedAt)}:{}),changeFrequency:'daily' as const,priority:.9}));
 ...
 return [...staticPages,...provincePages,...destinations,...destinationServices,...products,...stays.map(x=>detail('luu-tru',x.slug)),...tours.map(x=>detail('tour-du-lich',x.slug)),...cmsTours,...legacyCruises,...categories,...guides,...cmsGuides];
}
