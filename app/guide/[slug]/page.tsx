...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
 const {slug}=await params,cms=await getPublishedGuideSeo(slug),base=getSiteUrl(),url=`${base}/cam-nang/${encodeURIComponent(slug)}`,logo=`${base}/icon.svg`;
 if(cms){const title=cms.seoTitle||cms.title,description=cms.seoDescription||cms.excerpt;return{title:{absolute:title},description,alternates:{canonical:url},robots:{index:true,follow:true},openGraph:{title,description,url,type:'article',images:cms.cover?[{url:cms.cover,alt:cms.title}]:undefined},twitter:{card:'summary_large_image',title,description,images:cms.cover?[cms.cover]:undefined}}}
 ...
 return {title:{absolute:post.title},description:post.excerpt,keywords:post.keywords,alternates:{canonical:url},openGraph:{title:post.title,description:post.excerpt,url,type:'article',images:media.map(item=>({url:item.src,alt:item.alt||post.title})),siteName:'HappyGo Travel'},twitter:{card:'summary_large_image',title:post.title,description:post.excerpt,images:[media[0].src]}};
}
