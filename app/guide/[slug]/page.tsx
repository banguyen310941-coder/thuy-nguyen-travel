...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
 ...
 const base=getSiteUrl(),url=`${base}/guide/${post.slug}`;
...
export default async function GuideArticle({params}:{params:Promise<{slug:string}>}){
 ...
 const base=getSiteUrl(),url=`${base}/guide/${post.slug}`,allImages=[guideImage(post.image),...post.gallery.map(item=>guideImage(item.src))];
 const schema=[
