...
function count(terms:string[],cms:PublicGuideArticle[]){const cmsSlugs=new Set(cms.map(a=>a.slug));return cms.filter(a=>terms.some(t=>hay(a).includes(t.toLowerCase()))).length+guidePosts.filter(p=>!cmsSlugs.has(p.slug)&&terms.some(t=>hay(p).includes(t.toLowerCase()))).length}
...
export default async function GuidePage(){
 ...
 const cms=initialArticles.filter(a=>a.slug&&a.title&&a.cover),cmsSlugs=new Set(cms.map(a=>a.slug)),total=cms.length+guidePosts.filter(p=>!cmsSlugs.has(p.slug)).length;
 const schema=[
 ...
 return <div className="subpage guide-portal"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/>
  ...
  <section className="guide-portal-main"><div className="container guide-layout"><aside className="guide-sidebar"><div className="guide-side-card"><h3>Chủ đề cẩm nang</h3><Link className="active" href="/cam-nang"><span>Tất cả bài viết</span><b>{total}</b></Link>{guideCategories.map(category=><Link href={`/cam-nang/danh-muc/${category.slug}`} key={category.slug}><span>{category.name}</span><b>{count(category.terms,cms)}</b></Link>)}</div><div className="guide-side-card featured"><h3>Bài viết nổi bật</h3>{guidePosts.slice(0,4).map(p=><Link href={`/cam-nang/${p.slug}`} key={p.slug}><SafeImage src={guideImage(p.image)} alt={p.title}/><div><b>{p.title}</b><small>{p.date}</small></div></Link>)}</div><ContactQuickLink className="guide-side-cta" label="Tư vấn chuyến đi"/></aside><main className="guide-content"><div className="guide-section-head"><div><small>KINH NGHIỆM & GỢI Ý</small><h2>Bài viết cẩm nang mới</h2><p>Mọi bài SEO đã xuất bản được đưa thẳng vào cùng danh sách Cẩm nang và dùng chung một chuẩn hiển thị.</p></div><span>{total} bài viết</span></div><UnifiedGuideGrid staticItems={staticItems} initialArticles={initialArticles}/></main></div></section>
 </div>
}
