...
export type DatePriceSource={
  ...
  highSeasonRanges?:string|null;
  lowSeasonRanges?:string|null;
};
...
function monthDayNumber(value:string){const match=String(value||'').trim().match(/^(\d{2})-(\d{2})$/);return match?Number(match[1])*100+Number(match[2]):0}
export function dateInRecurringRanges(date:Date,ranges?:string|null){const current=(date.getMonth()+1)*100+date.getDate();return String(ranges||'').split(/[;,\n]+/).map(x=>x.trim()).filter(Boolean).some(range=>{const[startRaw,endRaw]=range.split(':').map(x=>x.trim()),start=monthDayNumber(startRaw),end=monthDayNumber(endRaw);if(!start||!end)return false;return start<=end?current>=start&&current<=end:current>=start||current<=end})}
export function configuredSeasonForDate(unit:DatePriceSource,date:Date):PricingSeason|null{if(dateInRecurringRanges(date,unit.highSeasonRanges))return'high';if(dateInRecurringRanges(date,unit.lowSeasonRanges))return'low';return null}
...
export function configuredUnitPrice(unit:DatePriceSource,date:Date){return seasonalUnitPrice(unit,date,configuredSeasonForDate(unit,date))}
