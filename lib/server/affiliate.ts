...
export async function findTrackableVilla(sql:any,villaId:string){const rows=await sql`select p.id,p.slug,p.name from products p where p.id=${villaId} and p.type='Villa & Resort' and ((p.partner_id is null and p.status='published') or (p.partner_id is not null and p.status='approved' and exists(select 1 from partners x where x.id=p.partner_id and x.status='active'))) limit 1`;return rows[0]||null}

export async function captureAffiliateReferral(sql:any,req:NextRequest,bookingId:string,customerPhone:string){try{const attr=readAffiliateAttribution(req);if(!attr)return null;const affiliate=(await sql`select id from affiliates where id=${attr.affiliateId} and status='active' limit 1`)[0];if(!affiliate)return null;const villa=await findTrackableVilla(sql,attr.villaId);if(!villa)return null;const rows=await sql`insert into affiliate_referrals(affiliate_id,booking_id,villa_id,customer_phone,status) values(${attr.affiliateId},${bookingId},${attr.villaId},${customerPhone||null},'pending') on conflict(booking_id) do nothing returning id`;return rows[0]?String(rows[0].id):null}catch(error){console.error('affiliate_referral_capture_failed',error);return null}}
...
export async function settleAffiliateBooking(sql:any,bookingId:string){const rows=await sql`
 with target as (
  ...
  from affiliate_referrals ar join affiliates a on a.id=ar.affiliate_id join bookings b on b.id=ar.booking_id
  where ar.booking_id=${bookingId} and ar.status='pending' and a.status='active' and b.status='completed'
  for update of ar
 ), credited as (
