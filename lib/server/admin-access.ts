...
export type AdminActor={id:string;name:string;email:string;role:string;department:string;permissions:string[]};
export async function adminActor(req:NextRequest,permission?:string):Promise<AdminActor|null>{
  ...
  const sql=db();
  const rows=await sql`select id,name,email,role,department,status,permissions from staff where id=${session.id} and status='active' limit 1`;
  ...
  return{id:String(row.id),name:String(row.name),email:String(row.email),role:String(row.role),department:String(row.department||''),permissions};
}
