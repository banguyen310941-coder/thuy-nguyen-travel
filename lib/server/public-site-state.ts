...
const PUBLIC_PRODUCT_FIELDS=['place','price','pricingBasis','summary','cover','gallery','rating','serviceStars','category','address','latitude','longitude','checkin','checkout','amenities','amenityTags','amenityDetails','policies','childrenPolicy','extraCharge','extraPersonFee','earlyCheckinFee','lateCheckoutFee','earlyLatePolicy','duration','pickup','boarding','route','itinerary','content','seoTitle','seoDescription','departure','airline','transport','departures','highlights','included','excluded','promotions','faq','childPrice','singleCharge','days','promotion','reviewCount'] as const;
const PUBLIC_UNIT_FIELDS=['code','name','bedrooms','beds','capacity','area','view','meal','amenities','weekdayPrice','weekendPrice','holidayPrice','lowWeekdayPrice','lowWeekendPrice','highWeekdayPrice','highWeekendPrice','extraAdult','extraChild','images','status','note','pricingBasis','guestType','priceGroup','ageLabel','ageMin','ageMax'] as const;
...
function articleTimestamp(item:any){const raw=String(item?.publishAt||item?.updatedAt||item?.date||'').trim();const vi=raw.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);if(vi)return Date.UTC(Number(vi[3]),Number(vi[2])-1,Number(vi[1]));const value=Date.parse(raw);return Number.isFinite(value)?value:0}
function newestFirst<T>(items:T[]){return[...items].sort((a:any,b:any)=>articleTimestamp(b)-articleTimestamp(a))}
...
function publicText(value:unknown){if(Array.isArray(value))return value.map(item=>customerText(item)).filter(Boolean).join('\n');return customerText(value)}
