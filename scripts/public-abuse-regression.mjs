...
must('lib/server/public-abuse.ts','const reader=req.body.getReader()','Bounded JSON phải đọc request body theo stream thay vì buffer toàn bộ trước');
must('lib/server/public-abuse.ts','totalBytes+=value.byteLength','Bounded JSON phải đếm byte thực tế theo từng chunk');
must('lib/server/public-abuse.ts','if(totalBytes>maxBytes)','Bounded JSON phải dừng ngay khi stream vượt giới hạn');
must('lib/server/public-abuse.ts','await reader.cancel().catch(()=>undefined)','Stream quá lớn phải được hủy ngay để tránh tiếp tục nhận body');
must('lib/server/public-abuse.ts','reader.releaseLock()','Reader phải luôn nhả lock sau khi đọc/hủy stream');
must('lib/server/public-abuse.ts','const bytes=new Uint8Array(totalBytes)','Chỉ được ghép body sau khi toàn bộ stream nằm trong giới hạn');
must('lib/server/public-abuse.ts','new TextDecoder().decode(bytes)','JSON chỉ được decode sau khi body đã qua giới hạn byte');
mustNot('lib/server/public-abuse.ts','await req.text()','Không được buffer toàn bộ request bằng req.text() trước khi kiểm giới hạn');
