...
const reviews='app/api/reviews/route.ts';
...
must(reviews,"al.entity_type='booking_account'",'Eligibility review phải tôn trọng marker booking/account');
must(reviews,"al.action in ('booking.account.linked','booking.account.unverified')",'Review phải hiểu trạng thái linked và unverified');
must(reviews,'order by al.created_at desc,al.id desc','Review phải dùng marker mới nhất để hỗ trợ xác minh sau này');
must(reviews,"'booking.account.legacy')<>'booking.account.unverified'",'Guest booking chưa xác minh không được cấp quyền review');
