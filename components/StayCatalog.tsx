...
export function StayCatalog(){
  ...
  const filtered=useMemo(()=>stays.filter(stay=>{
    ...
    const knownHotel=/vinpearl|flc|hotel|khách sạn/i.test(`${stay.slug} ${stay.name}`);
    const matchType=type==='all'||(type==='villa'&&t==='villa')||(type==='hotel'&&(t==='khách sạn'||knownHotel))||(type==='resort'&&t==='resort'&&!knownHotel);
  ...
  const label=type==='villa'?'Villa & Resort':type==='hotel'?'Khách sạn':type==='resort'?'Resort':'Lưu trú toàn quốc';
  return <>
    <div className="search-summary"><div><b>{q?`${label} cho “${params.get('q')}”`:label}</b><span>{filtered.length} lựa chọn · {checkin&&checkout?`${checkin} → ${checkout} · `:''}{adults} người lớn · {children} trẻ em · {rooms} phòng</span></div><Link href="/">Đổi tìm kiếm</Link></div>
    <div className="booking-layout">
      <aside className="booking-filter"><h3>Bộ lọc</h3><label>Loại chỗ nghỉ<select value={type} onChange={e=>setType(e.target.value)}><option value="all">Tất cả lưu trú</option><option value="villa">Villa</option><option value="hotel">Khách sạn</option><option value="resort">Resort khác</option></select></label><label>Điểm đánh giá<select value={rating} onChange={e=>setRating(e.target.value)}><option value="all">Tất cả</option><option value="9">9+ Tuyệt hảo</option><option value="8.5">8.5+ Rất tốt</option></select></label><label>Ngân sách<select defaultValue="all"><option value="all">Tất cả mức giá</option><option>Dưới 1.500.000đ</option><option>1.500.000đ – 3.000.000đ</option><option>Trên 3.000.000đ</option></select></label></aside>
