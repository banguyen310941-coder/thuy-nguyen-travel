...
export function StayCatalog(){
  ...
  const matches=(item:{name:string;location:string;summary:string;type:string;rating:number})=>{const text=`${item.name} ${item.location} ${item.summary}`.toLowerCase();const normalizedType=item.type.toLowerCase();const matchQ=!q||text.includes(q);const matchType=type==='all'||(type==='villa'&&normalizedType.includes('villa'))||(type==='hotel'&&(normalizedType.includes('khách sạn')||normalizedType.includes('resort')))||(type==='resort'&&normalizedType.includes('resort'));const matchRating=rating==='all'||item.rating>=Number(rating);return matchQ&&matchType&&matchRating};
