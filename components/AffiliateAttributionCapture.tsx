...
export function AffiliateAttributionCapture(){
 ...
 useEffect(()=>{
  ...
  const code=String(params.get('ref')||'').trim().toUpperCase(),productId=String(params.get('product_id')||params.get('villa_id')||'').trim();
  ...
  if(!code||!productId||!slug)return;
  const key=`${code}:${productId}:${slug}`;
  try{if(sessionStorage.getItem(`happygo-affiliate-track:${key}`))return}catch{}
  if(inFlight.current.has(key))return;
  inFlight.current.add(key);
  void fetch('/api/affiliate/track',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code,productId,villaId:productId,slug})}).then(response=>{
   if(!response.ok)throw new Error('affiliate_track_rejected');
   try{sessionStorage.setItem(`happygo-affiliate-track:${key}`,'1')}catch{}
   params.delete('ref');params.delete('product_id');params.delete('villa_id');
