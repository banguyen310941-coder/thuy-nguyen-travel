...
export function Header({settings}:{settings:SiteSettings}){
 ...
 useEffect(()=>{closeMobileMenu();setOpenProvinceKey(null);setOpenDesktopMenu(null)},[pathname]);
 ...
 return <header className="site-header mock-header happygo-header-v2">
  ...
  <div className="mock-nav-wrap"><nav className={`container mock-nav ${styles.desktopNav}`}>{nav.map(item=>item.provinces?.length?<div key={item.label} className={styles.navGroup} onMouseEnter={()=>setOpenDesktopMenu(item.label)} onMouseLeave={()=>{setOpenDesktopMenu(null);setOpenProvinceKey(null)}} onFocus={()=>setOpenDesktopMenu(item.label)}><Link href={item.href} aria-current={active(pathname,item.href)?'page':undefined} onClick={()=>setOpenDesktopMenu(null)}>{item.label}<span className={styles.chevron} data-open={openDesktopMenu===item.label?'true':'false'}>⌄</span></Link><div className={`${styles.dropdown} ${openDesktopMenu===item.label?styles.dropdownOpen:''}`}><div className={styles.dropdownHead}><b>{item.label} theo tỉnh / thành</b><Link href={item.href} onClick={()=>setOpenDesktopMenu(null)}>Xem tất cả →</Link></div>{item.provinces.map(province=>{const key=`desktop:${item.label}:${province.label}`;const hasChildren=Boolean(province.children?.length);const isOpen=openProvinceKey===key;return <div key={province.label} className={styles.provinceGroup}><div className={styles.provinceHeader}><Link href={province.href} className={styles.provinceLink} onClick={()=>setOpenDesktopMenu(null)}><span>{province.label}</span></Link>{hasChildren?<button type="button" className={styles.provinceToggle} aria-label={`${isOpen?'Thu gọn':'Mở'} điểm du lịch ${province.label}`} aria-expanded={isOpen} onClick={()=>toggleProvince(key)}><span>⌄</span></button>:null}</div>{hasChildren&&isOpen?<div className={styles.provinceChildren}>{province.children!.map(child=><Link key={child.href} href={child.href} className={styles.destinationLink} onClick={()=>setOpenDesktopMenu(null)}><span><b>{child.label}</b></span><em>›</em></Link>)}</div>:null}</div>})}</div></div>:<Link key={item.label} href={item.href} aria-current={active(pathname,item.href)?'page':undefined} onClick={()=>setOpenDesktopMenu(null)}>{item.label}</Link>)}</nav></div>
 </header>
}
