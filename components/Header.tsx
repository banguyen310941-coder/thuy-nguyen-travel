...
export function Header({settings}:{settings:SiteSettings}){const pathname=usePathname();const menuRef=useRef<HTMLDetailsElement>(null);const closeMobileMenu=()=>{if(menuRef.current)menuRef.current.open=false};useEffect(()=>{closeMobileMenu()},[pathname]);useEffect(()=>{const p=(e:PointerEvent)=>{const m=menuRef.current;if(m?.open&&!m.contains(e.target as Node))m.open=false};const k=(e:KeyboardEvent)=>{if(e.key==='Escape')closeMobileMenu()};document.addEventListener('pointerdown',p);document.addEventListener('keydown',k);return()=>{document.removeEventListener('pointerdown',p);document.removeEventListener('keydown',k)}},[]);const phone=settings.hotline.replace(/\D/g,'');const phoneLink=phoneHref(settings.hotline);return <header className="site-header mock-header happygo-header-v2"><div className="mock-contactbar"><div className="container mock-contactbar-inner"><div className="mock-mini-contact"><span>✉ {settings.email}</span>{phone&&phoneLink&&<a href={`tel:${phoneLink}`}>☎ {settings.hotline}</a>}</div><div className="mock-top-actions"><Link href="/cam-nang">Kinh nghiệm du lịch</Link><a href={`https://zalo.me/${settings.zalo.replace(/\D/g,'')}`} target="_blank" rel="noreferrer">Zalo</a><Link href="/partner">Đối tác</Link></div></div></div><div className="mock-mobile-head container"><details className="mobile-menu" ref={menuRef}><summary className="mock-mobile-icon" aria-label="Mở menu">☰</summary><nav className="mobile-menu-panel">{nav.map(item=><Link key={item.label} href={item.href} aria-current={active(pathname,item.href)?'page':undefined} onClick={closeMobileMenu}>{item.label}</Link>)}<Link href="/tai-khoan" onClick={closeMobileMenu}>👤 Tài khoản khách hàng</Link></nav></details><Link className="mock-mobile-logo happygo-link" href="/"><HappyGoLogo compact/></Link><Link className="mock-mobile-icon" href="/tai-khoan" aria-label="Tài khoản khách hàng">👤</Link></div><div className="container mock-brand-row"><Link className="mock-logo happygo-link" href="/"><HappyGoLogo/></Link><div className="mock-brand-tools"><Link href="/tim-kiem" className="mock-search-mini">⌕ Tìm điểm đến, khách sạn, tour...</Link><CustomerAccountBadge/></div></div><div className="mock-nav-wrap"><nav className="container mock-nav">{nav.map(item=><Link key={item.label} href={item.href} aria-current={active(pathname,item.href)?'page':undefined}>{item.label}</Link>)}</nav></div></header>}
