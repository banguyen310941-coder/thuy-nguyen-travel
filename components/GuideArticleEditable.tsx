...
const CONTENT_KEY='tn_cms_articles_v5';
...
export function GuideArticleEditable({post}:{post:StaticPost}){
 ...
 return <article className="public-article seo-static-article seo-guide-v2">
  ...
  <div className="seo-guide-shell">
   ...
   <div className="article-content seo-guide-content"><p className="seo-guide-summary"><strong>Tóm tắt:</strong> {excerpt}</p>{post.content.map((s,sectionIndex)=>{const photo=post.gallery[sectionIndex%post.gallery.length];return <section id={anchor(s.heading)} key={s.heading}><h2>{happyGoText(s.heading)}</h2>{s.paragraphs.map((x,i)=><p key={i}>{happyGoText(x)}</p>)}{photo&&sectionIndex<post.gallery.length&&<figure className="seo-guide-inline-photo"><img src={photo.src} alt={photo.alt} loading="lazy"/><figcaption>{photo.alt}{photo.credit?` · ${photo.credit}`:''}</figcaption></figure>}</section>})}</div>
  </div>
