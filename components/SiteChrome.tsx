...
import {formatPhone,useSiteSettings,type SiteSettings} from '@/components/useSiteSettings';
...
export function SiteChrome({children,initialSettings}:{children:React.ReactNode;initialSettings?:Partial<SiteSettings>|null}){
  ...
  return <><AffiliateAttributionCapture/><SiteProductionStateSync/><ApprovedPartnerCatalogSync/><DemoTourSchedules/><Header settings={settings}/><main>{children}</main><Footer settings={settings}/><BookingCartBadge/><div className="floating-actions" aria-label="Liên hệ nhanh"><a className="call-float" href={`tel:${phone}`} aria-label={`Gọi ${formatPhone(settings.hotline)}`} title={`Gọi ${formatPhone(settings.hotline)}`}><span>☎</span><span className="contact-label"><small>Hotline tư vấn</small><b>{formatPhone(settings.hotline)}</b></span></a><a className="zalo-float" href={`https://zalo.me/${zalo}`} target="_blank" rel="noreferrer" aria-label="Chat Zalo" title="Chat Zalo"><span className="zalo-word">Zalo</span><span className="contact-label"><small>Tư vấn nhanh</small><b>Chat Zalo</b></span></a></div></>;
}
