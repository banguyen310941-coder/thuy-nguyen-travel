...
export function GuideArticleReader(){
 ...
 useEffect(()=>{try{const slug=new URLSearchParams(window.location.search).get('slug')||'';const items=JSON.parse(localStorage.getItem('tn_cms_articles_v2')||'[]') as Article[];setArticle(items.find(x=>x.slug===slug&&x.status==='published')||null)}catch{setArticle(null)}},[]);
