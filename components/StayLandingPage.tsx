...
const configs:Record<StayKind,{kicker:string;title:string;description:string;breadcrumb:string;path:string;schemaName:string;cta:string}>={
 ...
 villa:{kicker:'VILLA TOÀN QUỐC',title:'Villa',description:'Khám phá villa nguyên căn theo điểm đến, sức chứa, tiện ích và ngân sách. Xem đúng căn, đúng chính sách và kiểm tra lịch trước khi đặt.',breadcrumb:'Villa',path:'/villa',schemaName:'Villa toàn quốc',cta:'Tư vấn Villa'},
 hotel:{kicker:'KHÁCH SẠN & RESORT TOÀN QUỐC',title:'Khách sạn & Resort',description:'Tìm khách sạn và resort theo điểm đến, hạng phòng, số khách và nhu cầu chuyến đi. Giá phòng và chính sách được hiển thị rõ để bạn dễ so sánh.',breadcrumb:'Khách sạn & Resort',path:'/khach-san-resort',schemaName:'Khách sạn & Resort toàn quốc',cta:'Tư vấn Khách sạn & Resort'},
};

export async function StayLandingPage({kind='all',query={}}:{kind?:StayKind;query?:PublicCatalogQuery}){
 const cfg=configs[kind],base=getSiteUrl(),rawDestination=Array.isArray(query.q)?String(query.q[0]||''):String(query.q||''),destination=getSeoDestination(rawDestination)?.name||rawDestination,destinationKey=destinationSlug(destination),publicPath=destinationKey&&kind!=='all'?`/diem-den/${destinationKey}/${kind==='villa'?'villa':'khach-san-resort'}`:cfg.path,canonical=`${base}${publicPath}`,effectiveQuery=destination?{...query,q:destination}:query;
 ...
 return <div className="subpage"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/>{kind==='villa'?<section className="sub-section white"><div className="container"><SearchBar/></div></section>:null}<section className="sub-hero"><div className="container"><div className="sub-breadcrumb"><Link href="/">Trang chủ</Link> / {destinationKey&&kind!=='all'?<><Link href="/diem-den">Điểm đến</Link> / <Link href={`/diem-den/${destinationKey}`}>{destination}</Link> / {cfg.breadcrumb}</>:cfg.breadcrumb}</div><div className="sub-hero-grid"><div><span className="sub-kicker">{destination?`LƯU TRÚ ${destination.toUpperCase()}`:cfg.kicker}</span><h1>{destination?`${cfg.title} tại ${destination}`:cfg.title}</h1><p>{destination?`Danh sách ${cfg.breadcrumb.toLowerCase()} phù hợp tại ${destination}. ${cfg.description}`:cfg.description}</p></div><ContactCtaGroup callLabel={cfg.cta}/></div></div></section><PublicServiceNav active={kind==='villa'?'villa':kind==='hotel'?'hotel':'stay'} destination={destination}/>{kind!=='villa'?<section className="sub-section white"><div className="container"><SearchBar/></div></section>:null}<section className="sub-section"><div className="container"><StayCatalog initialType={initialType} initialCms={initialCms} query={effectiveQuery}/><div className="sub-cta"><div><h2>Cần kiểm tra phòng nhanh?</h2><p>Gửi ngày ở, số khách và điểm đến để HappyGo Travel kiểm tra phương án phù hợp.</p></div><ContactCtaGroup mode="footer" zaloLabel="Chat Zalo"/></div></div></section></div>
}
