...
export async function StayLandingPage({kind='all',query={}}:{kind?:StayKind;query?:PublicCatalogQuery}){
 const cfg=configs[kind],base=getSiteUrl(),canonical=`${base}${cfg.path}`,destination=Array.isArray(query.q)?String(query.q[0]||''):String(query.q||'');
 ...
 return <div className="subpage"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/><section className="sub-hero"><div className="container"><div className="sub-breadcrumb"><Link href="/">Trang chủ</Link> / {cfg.breadcrumb}</div><div className="sub-hero-grid"><div><span className="sub-kicker">{destination?`LƯU TRÚ ${destination.toUpperCase()}`:cfg.kicker}</span><h1>{destination?`${cfg.title} tại ${destination}`:cfg.title}</h1><p>{destination?`Danh sách ${cfg.breadcrumb.toLowerCase()} phù hợp tại ${destination}. ${cfg.description}`:cfg.description}</p></div><ContactCtaGroup callLabel={cfg.cta}/></div></div></section><nav className="sub-nav" aria-label="Danh mục lưu trú"><div className="container sub-nav-inner"><Link className={kind==='all'?'active':undefined} href={destination?`/luu-tru?q=${encodeURIComponent(destination)}`:'/luu-tru'}>Tất cả lưu trú</Link><Link className={kind==='villa'?'active':undefined} href={destination?`/villa-resort?q=${encodeURIComponent(destination)}`:'/villa-resort'}>Villa & Resort</Link><Link className={kind==='hotel'?'active':undefined} href={destination?`/khach-san?q=${encodeURIComponent(destination)}`:'/khach-san'}>Khách sạn</Link><Link href={destination?`/diem-den/${encodeURIComponent(destination.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/đ/g,'d').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''))}`:'/diem-den'}>Điểm đến</Link><Link href="/cam-nang">Cẩm nang</Link></div></nav><section className="sub-section white"><div className="container"><SearchBar/></div></section><section className="sub-section"><div className="container"><StayCatalog initialType={initialType} initialCms={initialCms} query={query}/><div className="sub-cta"><div><h2>Cần kiểm tra phòng nhanh?</h2><p>Gửi ngày ở, số khách và điểm đến để HappyGo Travel kiểm tra phương án phù hợp.</p></div><ContactCtaGroup mode="footer" zaloLabel="Chat Zalo"/></div></div></section></div>
}
