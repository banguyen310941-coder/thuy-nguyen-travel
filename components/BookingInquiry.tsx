...
export function BookingInquiry({product,kind='dịch vụ'}:{product:string;kind?:string}){
  ...
  async function send(form:HTMLFormElement){
    ...
    if(name.length<2){setMessage('Vui lòng nhập họ tên đầy đủ.');return}if(!/^(0|\+84)\d{9,10}$/.test(phone)){setMessage('Số điện thoại chưa đúng định dạng Việt Nam.');return}if(from&&from<today){setMessage('Ngày đi/nhận phòng không thể ở trong quá khứ.');return}if(isStay&&from&&to&&to<=from){setMessage('Ngày trả phòng phải sau ngày nhận phòng.');return}
    ...
    const payload={kind,product,customerName:name,phone,email:String(data.get('email')||'').trim(),startDate:from||null,endDate:isStay?(to||null):null,adults:Number(data.get('adults')||2),children:Number(data.get('children')||0),rooms:isTour?1:Number(data.get('rooms')||1),note,source:'website'};
    let apiFailed=false;
    if(API_BASE){try{setState('sending');const response=await fetch(`${API_BASE.replace(/\/$/,'')}/api/bookings`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!response.ok)throw new Error('Booking API error');const result=await response.json();setCode(result.code||'');setState('saved');form.reset();setSelectedUnit('');return}catch(error){console.error(error);apiFailed=true}}
    const unitLabel=isCruise?'Số cabin':isStay?'Số phòng':'';
    const text=[`YÊU CẦU TƯ VẤN ${kind.toUpperCase()}`,`Sản phẩm: ${product}`,selectedUnit?`Đang chọn: ${selectedUnit}`:'',`Khách hàng: ${payload.customerName}`,`Điện thoại: ${payload.phone}`,`Email: ${payload.email||'Không cung cấp'}`,`Ngày đi/nhận: ${payload.startDate||'Chưa chọn'}`,isStay?`Ngày trả: ${payload.endDate||'Chưa chọn'}`:'',`Người lớn: ${payload.adults}`,`Trẻ em: ${payload.children}`,unitLabel?`${unitLabel}: ${payload.rooms}`:'',`Ghi chú: ${rawNote||'Không có'}`].filter(Boolean).join('\n');
