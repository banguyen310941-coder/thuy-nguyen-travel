...
const VERSION='villa-seo-2026-09-05-v3-exact-product-images';
...
const toHtml=(post:(typeof guidePosts)[number])=>{
 ...
 return [`<p><strong>Tóm tắt:</strong> ${esc(post.excerpt)}</p>`,...post.content.flatMap((section,index)=>{const images=slots.get(index)||[];return [`<h2>${esc(section.heading)}</h2>`,...section.paragraphs.map(p=>`<p>${esc(p)}</p>`),...images.map(image=>`<figure><img src="${image.src}" alt="${esc(image.alt)}" loading="lazy"/><figcaption><strong>${esc(image.alt)}</strong>${image.credit?` · ${esc(image.credit)}`:''}</figcaption></figure>`)]}),'<h2>Câu hỏi thường gặp</h2>',...post.faq.flatMap(item=>[`<h3>${esc(item.q)}</h3>`,`<p>${esc(item.a)}</p>`])].join('\n')
};
