...
export function DailyPriceRange({slug,fallback='',checkin='',checkout='',compact=false}:{slug:string;fallback?:string;checkin?:string|null;checkout?:string|null;compact?:boolean}){
 ...
 const range=useMemo(()=>{const active=(product?.units||[]).filter(u=>u.status!=='hidden'&&u.status!=='soldout');const vals:number[]=[];if(checkin){const start=new Date(`${checkin}T12:00:00`);const end=checkout?new Date(`${checkout}T12:00:00`):new Date(start.getFullYear(),start.getMonth(),start.getDate()+1);for(const u of active){for(let d=new Date(start);d<end;d=new Date(d.getFullYear(),d.getMonth(),d.getDate()+1)){const p=priceForDate(u,d);if(p)vals.push(p)}}}else{for(const u of active){[money(u.weekdayPrice),money(u.weekendPrice),money(u.holidayPrice)].filter(Boolean).forEach(v=>vals.push(v))}}return vals.length?{min:Math.min(...vals),max:Math.max(...vals)}:null},[product,checkin,checkout,rev]);
