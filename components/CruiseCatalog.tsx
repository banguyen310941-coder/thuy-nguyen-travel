...
export function CruiseCatalog(){
  ...
  useEffect(()=>{const load=()=>{try{const raw=JSON.parse(localStorage.getItem('tn_cms_products_v3_units')||'[]') as CmsProduct[];const staticSlugs=new Set(cruises.map(x=>x.slug));setCms(raw.filter(x=>x.status==='published'&&x.type==='Du thuyền'&&x.slug&&!staticSlugs.has(x.slug)))}catch{setCms([])}};load();window.addEventListener('tn-products-updated',load);return()=>window.removeEventListener('tn-products-updated',load)},[]);
