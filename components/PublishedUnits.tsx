...
export function PublishedUnits({slug,label='Căn / hạng phòng'}:{slug:string;label?:string}){
 ...
 return <section className="detail-block live-units" id="units"><div className="live-units-head"><h2>{label} đang quản lý</h2><p>Giá hiển thị theo từng căn/phòng; ngày lễ và tình trạng trống cần xác nhận theo ngày thực tế.</p></div><div className="live-unit-list">{units.map(u=><article key={u.id}><div><b>{u.name}</b><small>{u.code||'Chưa có mã'} · {u.capacity||'Sức chứa liên hệ'}{u.area?` · ${u.area}`:''}{u.view?` · ${u.view}`:''}</small>{u.meal&&<span>{u.meal}</span>}</div><div className="live-unit-prices"><span><small>Ngày thường</small><b>{u.weekdayPrice||'Liên hệ'}</b></span><span><small>Cuối tuần</small><b>{u.weekendPrice||u.weekdayPrice||'Liên hệ'}</b></span><span><small>Lễ/Tết</small><b>{u.holidayPrice||'Liên hệ'}</b></span></div><em className={`unit-public-status ${u.status}`}>{u.status==='available'?'Còn bán':u.status==='hold'?'Tạm giữ':u.status==='soldout'?'Hết phòng':'Ẩn'}</em><a href="#booking">Đặt căn này</a></article>)}</div></section>;
}
