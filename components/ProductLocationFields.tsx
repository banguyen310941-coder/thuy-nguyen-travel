...
export function ProductLocationFields({form,setField,kind}:Props){
 ...
 return <section className="pm-section product-location-editor"><div className="product-flow-heading"><div><small>VỊ TRÍ CHÍNH XÁC · DÙNG CHO BẢN ĐỒ & TÌM GẦN ĐÂY</small><h3>Vị trí & ghim bản đồ *</h3><p>Nhập khu vực, địa chỉ và ghim chính xác như luồng đặt phòng. Sản phẩm sẽ không thể đăng nếu chưa có đủ vị trí.</p></div><span className={`product-flow-badge ${issues.length?'location-missing':'location-ready'}`}>{issues.length?`Thiếu ${issues.length} mục`:'✓ Đủ vị trí'}</span></div>
