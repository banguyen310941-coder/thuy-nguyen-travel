...
export function ProductLocationFields({form,setField,kind}:Props){
 ...
 return <section className="pm-section product-location-editor"><div className="product-flow-heading"><div><small>VỊ TRÍ CHÍNH XÁC · DÙNG CHO BẢN ĐỒ & TÌM GẦN ĐÂY</small><h3>Ghim vị trí trên bản đồ *</h3><p>Sau khi nhập khu vực và {kind==='cruise'?'bến khởi hành':'địa chỉ'} ở trên, ghim tọa độ chính xác. Sản phẩm sẽ không thể đăng nếu chưa đủ vị trí.</p></div><span className={`product-flow-badge ${issues.length?'location-missing':'location-ready'}`}>{issues.length?`Thiếu ${issues.length} mục`:'✓ Đủ vị trí'}</span></div>
