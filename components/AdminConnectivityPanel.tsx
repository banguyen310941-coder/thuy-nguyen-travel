...
export function AdminConnectivityPanel(){
  ...
  return <section className="admin-panel admin-connectivity-panel"><div className="admin-panel-head"><div><h2>Kiểm tra kết nối dữ liệu</h2><p>Đối chiếu nhanh dữ liệu Admin đang có trên thiết bị và phát tín hiệu cập nhật cho giao diện.</p></div><button type="button" onClick={refreshAll}>↻ Làm mới kết nối</button></div><div className="admin-connect-summary"><span><b>{healthy}/{rows.length}</b> module có dữ liệu</span><span className={invalid?'bad':'good'}>{invalid?`${invalid} module dữ liệu lỗi`:'Không phát hiện dữ liệu hỏng'}</span>{lastRefresh&&<small>Làm mới lúc {lastRefresh}</small>}</div><div className="admin-connect-grid">{rows.map(row=><article key={row.key} className={`admin-connect-item ${row.state}`}><div><b>{row.label}</b><small>{row.key}</small></div><strong>{row.state==='invalid'?'Lỗi':row.count}</strong><span>{row.state==='ok'?'Đã kết nối':row.state==='empty'?'Chưa có dữ liệu':'JSON không hợp lệ'}</span></article>)}</div><p className="admin-connect-note">Booking và dữ liệu vận hành được đồng bộ với cơ sở dữ liệu chung khi huy hiệu trên thanh quản trị báo “Đã đồng bộ”. Nội dung CMS vẫn dùng bản lưu trên trình duyệt ở giai đoạn này.</p></section>;
}
