  ...
  <div className="affiliate-toolkit-grid">
   <div className="affiliate-toolkit-builder">
    ...
    {product?<>
     ...
     <div className="affiliate-media-head"><div><b>Album ảnh gửi khách</b><small>{media.length?`${media.length} ảnh lấy trực tiếp từ dữ liệu sản phẩm.`:'Sản phẩm này chưa có ảnh nguồn.'}</small></div>{product.albumUrl&&<a href={product.albumUrl} target="_blank" rel="noreferrer">Mở thư mục Drive gốc ↗</a>}</div>
     {media.length?<div className="affiliate-media-grid">{media.map((src,index)=><article key={`${src}_${index}`}><a href={src} target="_blank" rel="noreferrer" className="affiliate-media-preview"><img src={src} alt={`${product.name} - ảnh ${index+1}`} loading="lazy"/></a><div><span>Ảnh {index+1}</span><a href={downloadUrl(src)} target="_blank" rel="noreferrer">Tải ảnh</a></div></article>)}</div>:<div className="affiliate-empty">Chưa có album ảnh cho sản phẩm này.</div>}
