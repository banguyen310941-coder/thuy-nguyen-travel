...
type Sort='attention'|'commission'|'conversion'|'clicks';
...
export function AdminAffiliatePerformance(){
 ...
 const[data,setData]=useState<Payload|null>(null),[busy,setBusy]=useState(false),[msg,setMsg]=useState(''),[q,setQ]=useState(''),[filter,setFilter]=useState('all'),[sort,setSort]=useState<Sort>('attention');
 ...
 const visible=useMemo(()=>{
  ...
  return [...rows].sort((a,b)=>sort==='commission'?b.commission30-a.commission30:sort==='conversion'?b.conversionRate-a.conversionRate:sort==='clicks'?b.clicks30-a.clicks30:b.attentionScore-a.attentionScore||b.commission30-a.commission30);
 },[data,q,filter,sort]);
 const s=data?.summary;
 return <section className="admin-panel affiliate-admin">
  <div className="admin-panel-head"><div><small>HIỆU SUẤT CTV · 30 NGÀY</small><h2>Leaderboard, chuyển đổi & CTV cần chăm sóc</h2><p>{data?.scope==='assigned'?'Sale chỉ xem hiệu suất các CTV đang được giao cho mình.':'Admin/Owner xem hiệu suất toàn mạng lưới CTV.'} Số liệu không chứa SĐT khách hoặc thông tin ngân hàng.</p></div><button type="button" className="admin-secondary" onClick={()=>void load()} disabled={busy}>{busy?'Đang cập nhật...':'↻ Làm mới'}</button></div>
  ...
  <div className="tour-editor-grid"><label className="span-2">Tìm CTV<input value={q} onChange={e=>setQ(e.target.value)} placeholder="Tên, mã CTV, Sale phụ trách hoặc cảnh báo"/></label><label>Sắp xếp<select value={sort} onChange={e=>setSort(e.target.value as Sort)}><option value="attention">Ưu tiên cần xử lý</option><option value="commission">Hoa hồng cao nhất</option><option value="conversion">Chuyển đổi cao nhất</option><option value="clicks">Click cao nhất</option></select></label></div>
  <div className="affiliate-admin-table"><table><thead><tr><th>CTV</th><th>Sale phụ trách</th><th>Click 30N</th><th>Booking 30N</th><th>Chuyển đổi</th><th>Hoa hồng 30N</th><th>Payout chờ</th><th>Follow-up</th><th>Ưu tiên</th></tr></thead><tbody>{visible.map(item=><tr key={item.id}><td><b>{item.name}</b><small>{item.referralCode} · {statusLabel(item.status)}</small></td><td>{item.salesOwnerName}</td><td>{number(item.clicks30)}</td><td><b>{number(item.bookings30)}</b><small>{item.creditedOrders30} đã ghi hoa hồng · {item.pendingOrders} chờ</small></td><td><b>{item.conversionRate}%</b></td><td><b>{money(item.commission30)}</b><small>{money(item.paidPayouts30)} payout 30N</small></td><td>{item.pendingPayouts?<b>{money(item.pendingPayouts)}</b>:'—'}</td><td><span className={item.nextFollowUpAt&&new Date(item.nextFollowUpAt).getTime()<Date.now()?'status-cancelled':'status-confirmed'}>{item.nextFollowUpAt?fmt(item.nextFollowUpAt):'Chưa hẹn'}</span><small>{item.lastFollowupAt?`Gần nhất ${fmt(item.lastFollowupAt)}`:'Chưa chăm sóc'}</small></td><td>{item.attentionReasons.length?<div>{item.attentionReasons.map(reason=><span key={reason} className="status-pending">{reason}</span>)}</div>:<span className="status-confirmed">Ổn định</span>}</td></tr>)}{!visible.length&&!busy&&<tr><td colSpan={9}>Không có CTV phù hợp bộ lọc.</td></tr>}{busy&&!data&&<tr><td colSpan={9}>Đang tổng hợp hiệu suất CTV...</td></tr>}</tbody></table></div>
 </section>;
}
