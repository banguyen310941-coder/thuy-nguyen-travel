...
export function AdminAffiliatePayoutRequests(){
 ...
 return <section className="admin-panel affiliate-payout-queue">
  <div className="admin-panel-head"><div><small>CTV · YÊU CẦU RÚT HOA HỒNG</small><h2>Hàng chờ đối soát thanh toán</h2><p>Chỉ trừ số dư ví sau khi Admin bấm duyệt thanh toán.</p></div><span className="affiliate-payout-count">{items.length} đang chờ</span></div>
  ...
  <div className="affiliate-payout-grid">{items.map(p=><article key={p.id}>
   ...
   <div className="affiliate-payout-buttons"><button type="button" onClick={()=>void resolve(p,'cancelled')} disabled={busy}>Từ chối</button><button type="button" className="approve" onClick={()=>void resolve(p,'paid')} disabled={busy}>✓ Duyệt thanh toán</button></div>
  </article>)}</div>
 </section>;
}
