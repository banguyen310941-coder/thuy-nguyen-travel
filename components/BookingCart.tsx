...
export function BookingCartBadge(){const[count,setCount]=useState(0);useEffect(()=>{const load=()=>setCount(readCart().length);load();window.addEventListener('tn-cart-updated',load);window.addEventListener('storage',load);return()=>{window.removeEventListener('tn-cart-updated',load);window.removeEventListener('storage',load)}},[]);const stateClass=count>0?'cart-has-items':'cart-empty';return <Link className={`booking-cart-badge ${stateClass}`} href="/thanh-toan" aria-label={`Giỏ đặt dịch vụ có ${count} mục`}><span>🧳</span><b>Đặt dịch vụ</b>{count>0&&<em>{count}</em>}</Link>}
