...
const RATE_CACHE_KEY='tn_cms_daily_rates_v1';
const rateKey=(r:PublicRateRange)=>`${r.unitId||''}|${r.start||''}|${r.end||''}`;
function cacheRates(next:PublicRateRange[]){if(!next.length)return;try{const current=JSON.parse(localStorage.getItem(RATE_CACHE_KEY)||'[]');const merged=new Map<string,PublicRateRange>();for(const rate of Array.isArray(current)?current:[])merged.set(rateKey(rate as PublicRateRange),rate as PublicRateRange);for(const rate of next)merged.set(rateKey(rate),rate);localStorage.setItem(RATE_CACHE_KEY,JSON.stringify([...merged.values()]))}catch{}}
const empty=(p:PartnerPublicProduct,price:string):PublicProduct=>{const canonical=canonicalPartnerProduct(p);return{...canonical,price:price||canonical.price,rating:String(p.rating||''),unitsText:p.unitsText||'',departure:p.departure,route:p.route||canonical.route||'',airline:p.airline,transport:p.transport,departures:p.departures,included:p.included,excluded:p.excluded,highlights:p.highlights,promotions:p.promotions} as PublicProduct};
