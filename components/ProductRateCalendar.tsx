...
import {fallbackUnitPrice,pricingDateKey,pricingMoney} from '@/lib/pricing-calendar';
...
export function ProductRateCalendar({units,label='Lịch giá theo ngày'}:{units:Unit[];label?:string}){
  ...
  return <section id="rate-calendar" className="pd-block public-rate-calendar">
    ...
    <div className="prc-grid">{cells.map((date,index)=>{if(!date)return <span className="prc-empty" key={`empty_${index}`}/>;const key=pricingDateKey(date);const rate=rateFor(key);const fromCalendar=rate&&rate.status==='available'&&Number(rate.quantity||0)>0?pricingMoney(rate.price):rate?0:fallbackUnitPrice(unit as any,date);const unavailable=Boolean(rate&&(rate.status!=='available'||Number(rate.quantity||0)<=0));return <div className={`${key<today?'past':''} ${unavailable?'soldout':''}`} key={key}><small>{date.getDate()}</small><b>{unavailable?'Hết':fromCalendar?compact(fromCalendar):'Liên hệ'}</b>{rate?.note&&<em title={rate.note}>{rate.note}</em>}</div>})}</div>
    <div className="prc-note"><span>Giá hiển thị theo 1 đêm.</span><a href="#booking">Chọn ngày trong khung đặt phòng →</a></div>
  </section>;
}
