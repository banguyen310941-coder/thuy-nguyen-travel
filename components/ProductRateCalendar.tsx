...
export function ProductRateCalendar({units,label='Lịch giá theo ngày'}:{units:Unit[];label?:string}){
  ...
  const selected=params.get('checkin')||'';
  ...
  return <section id="rate-calendar" className="pd-block public-rate-calendar">
    ...
    <div className="prc-grid">{cells.map((date,index)=>{
      ...
      const disabled=key<today||unavailable||missing;
      const className=[key<today?'past':'',unavailable?'soldout':'',missing?'unpriced':'',key===selected?'selected':'',!disabled?'clickable':''].filter(Boolean).join(' ');
      const status=unavailable?'Hết':missing?'Chưa mở':compact(price);
      return <button type="button" className={className} key={key} disabled={disabled} onClick={()=>chooseDate(date,price,unavailable)} title={missing?'Ngày này chưa có giá bán chính xác trong lịch production':unavailable?'Ngày này hiện không còn bán':`Chọn ngày ${key} · ${new Intl.NumberFormat('vi-VN').format(price)}đ`}><small>{date.getDate()}</small><b>{status}</b>{rate?.note&&<em>{rate.note}</em>}</button>;
    })}</div>
