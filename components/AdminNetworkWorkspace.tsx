...
import {AdminAffiliatePerformance} from '@/components/AdminAffiliatePerformance';
import {AdminAffiliateFollowups} from '@/components/AdminAffiliateFollowups';
import {AdminAffiliatePayoutQueue} from '@/components/AdminAffiliatePayoutQueue';
...
const tabs:TabItem[]=[{id:'overview',label:'Tổng quan',hint:'Việc chờ xử lý',area:'all'},{id:'partners',label:'Đối tác',hint:'Hồ sơ & sản phẩm',area:'partners'},{id:'support',label:'Hỗ trợ',hint:'Trao đổi đối tác',area:'partners'},{id:'affiliates',label:'CTV / Affiliate',hint:'Hồ sơ, Sale & tài chính',area:'affiliates'}];
const allowedTab=(item:TabItem,access:Access)=>item.area==='all'||(item.area==='partners'&&access.canPartners)||(item.area==='affiliates'&&(access.canAffiliates||access.canAffiliateFinance));
...
export function AdminNetworkWorkspace(){
 ...
 const financeOnly=access.canAffiliateFinance&&!access.canAffiliates;
 return <section className="admin-network-workspace"><div className="admin-welcome production"><div><small>MẠNG LƯỚI HỢP TÁC · PRODUCTION</small><h2>Đối tác & Cộng tác viên</h2><p>{financeOnly?'Tài khoản Tài chính CTV chỉ được truy cập hoa hồng, đối soát và thanh toán; hồ sơ, phân công Sale và lịch chăm sóc được ẩn.':'Hồ sơ CTV, Sale phụ trách, hiệu suất bán hàng, lịch chăm sóc và tài chính CTV được tách quyền độc lập; chỉ hiển thị dữ liệu đúng quyền của tài khoản.'}</p></div><div className="admin-production-badge"><i/>Production secure</div></div>{accessError&&<div className="admin-connect-error">{accessError}</div>}<nav className="admin-network-tabs" aria-label="Mạng lưới hợp tác">{visibleTabs.map(item=><button key={item.id} type="button" className={tab===item.id?'active':''} onClick={()=>choose(item.id)}><b>{item.label}</b><small>{item.id==='affiliates'&&financeOnly?'Hoa hồng & thanh toán':item.hint}</small></button>)}</nav>{tab==='overview'&&<><AdminNetworkStatusLegend/><AdminNetworkOperations canPartners={access.canPartners} canAffiliates={access.canAffiliates} canAffiliateFinance={access.canAffiliateFinance}/></>}{tab==='partners'&&access.canPartners&&<AdminPartnerManager/>}{tab==='support'&&access.canPartners&&<PartnerSupportCenter/>}{tab==='affiliates'&&(access.canAffiliates||access.canAffiliateFinance)&&<>{access.canAffiliates&&<><AdminAffiliateAssignments/><AdminAffiliatePerformance/><AdminAffiliateFollowups/></>}{access.canAffiliateFinance&&<AdminAffiliatePayoutQueue/>}<AdminAffiliateManager canManage={access.canAffiliates} canFinance={access.canAffiliateFinance}/></>}</section>;
}
