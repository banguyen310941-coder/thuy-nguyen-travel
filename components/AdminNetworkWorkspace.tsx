...
const tabs:TabItem[]=[
  ...
  {id:'affiliates',label:'CTV / Affiliate',hint:'Hồ sơ & hoa hồng',area:'affiliates'},
];
...
export function AdminNetworkWorkspace(){
  ...
  const loadAccess=useCallback(async()=>{
    try{
      ...
      setAccess({
        canPartners:elevated||permissions.includes('partners'),
        canAffiliates:elevated||permissions.includes('affiliates'),
        canAffiliateFinance:elevated||permissions.includes('affiliate_finance'),
      });
      setAccessError('');
    }catch(error){setAccess({canPartners:false,canAffiliates:false,canAffiliateFinance:false});setAccessError(error instanceof Error?error.message:'Không đọc được quyền mạng lưới.')}
  },[]);
  ...
  return <section className="admin-network-workspace">
    <div className="admin-welcome production">
      <div><small>MẠNG LƯỚI HỢP TÁC · PRODUCTION</small><h2>Đối tác & Cộng tác viên</h2><p>Hồ sơ CTV và tài chính CTV được tách quyền độc lập; chỉ hiển thị dữ liệu đúng quyền của tài khoản.</p></div>
    ...
    {tab==='overview'&&<><AdminNetworkStatusLegend/><AdminNetworkOperations canPartners={access.canPartners} canAffiliates={access.canAffiliates} canAffiliateFinance={access.canAffiliateFinance}/></>}
    {tab==='partners'&&access.canPartners&&<AdminPartnerManager/>}
    {tab==='support'&&access.canPartners&&<PartnerSupportCenter/>}
    {tab==='affiliates'&&access.canAffiliates&&<AdminAffiliateManager canFinance={access.canAffiliateFinance}/>}
  </section>;
}
