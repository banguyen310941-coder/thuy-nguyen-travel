...
export function HomeCmsSections(){
  ...
  return <>
    ...
    {cms.productsEnabled&&<section className="mock-section mock-soft"><div className="container"><div className="mock-section-heading"><div><h2>{cms.productsTitle}</h2><div className="mock-pills"><Link className="active" href="/stay?type=villa">Villa & Resort</Link><Link href="/stay?type=hotel">Khách sạn</Link><Link href="/cruises">Du thuyền</Link><Link href="/tours">Tour du lịch</Link></div></div><Link href="/stay">Xem tất cả →</Link></div><div className="mock-product-grid">{stays.slice(0,5).map(stay=><PropertyCard stay={stay} key={stay.slug}/>)}</div></div></section>}
