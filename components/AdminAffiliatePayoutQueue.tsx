...
type Payout={id:string;affiliateId:string;affiliateName:string;amount:number;status:string;payoutDate:string;receiptUrl:string;createdAt:string;bankAccount:string;bankName:string;accountHolder:string};
type Payload={financeAccess?:boolean;payouts:Payout[]};
...
export function AdminAffiliatePayoutQueue(){
 ...
 const pending=useMemo(()=>[...(data?.payouts||[])].filter(p=>p.status==='pending').sort((a,b)=>new Date(a.createdAt).getTime()-new Date(b.createdAt).getTime()),[data]);
 const totalPending=useMemo(()=>pending.reduce((total,p)=>total+p.amount,0),[pending]);
 async function resolve(payout:Payout,decision:Decision){
  ...
  }catch(error){setMsg(error instanceof Error?error.message:'Không xử lý được payout CTV.')}finally{decisionLocks.current.delete(payout.id);setProcessing(current=>current.filter(id=>id!==payout.id))}
 }
 return <section className="admin-panel affiliate-admin">
  <div className="admin-panel-head"><div><small>CTV PAYOUT QUEUE · PRODUCTION</small><h2>Yêu cầu rút tiền đang chờ</h2><p>Duyệt hoặc hủy payout ngay tại đây. Khi một CTV còn payout pending, API thanh toán trực tiếp vẫn từ chối giao dịch mới cho tới khi hàng chờ được xử lý.</p></div><button type="button" className="admin-secondary" onClick={()=>void load()} disabled={busy}>{busy?'Đang cập nhật...':'↻ Làm mới'}</button></div>
  {msg&&<p className="admin-api-note" aria-live="polite">{msg}</p>}
  <div className="affiliate-admin-kpis"><article><small>YÊU CẦU ĐANG CHỜ</small><b>{pending.length}</b><span>cần quyết định</span></article><article><small>TỔNG TIỀN ĐANG CHỜ</small><b>{money(totalPending)}</b><span>chưa trừ số dư cho tới khi duyệt</span></article><article><small>YÊU CẦU CŨ NHẤT</small><b>{pending[0]?dateTime(pending[0].createdAt):'—'}</b><span>ưu tiên xử lý trước</span></article></div>
  <div className="affiliate-admin-table"><table><thead><tr><th>CTV</th><th>Số tiền</th><th>Tài khoản nhận</th><th>Tạo lúc</th><th>Thao tác</th></tr></thead><tbody>{pending.map(p=>{const locked=processing.includes(p.id);return <tr key={p.id}><td><b>{p.affiliateName}</b><small>{p.id.slice(0,8)}…</small></td><td><b>{money(p.amount)}</b></td><td><b>{p.bankName||'Chưa có ngân hàng'}</b><small>{p.bankAccount||'—'} · {p.accountHolder||'Chưa có chủ tài khoản'}</small></td><td>{dateTime(p.createdAt)}</td><td><div className="pm-actions"><button type="button" className="admin-primary" onClick={()=>void resolve(p,'paid')} disabled={locked}>Duyệt chi</button><button type="button" className="danger-action" onClick={()=>void resolve(p,'cancelled')} disabled={locked}>Hủy yêu cầu</button></div></td></tr>})}{!pending.length&&!busy&&<tr><td colSpan={5}>Không có payout CTV đang chờ.</td></tr>}{busy&&!data&&<tr><td colSpan={5}>Đang tải hàng chờ payout...</td></tr>}</tbody></table></div>
 </section>;
}
