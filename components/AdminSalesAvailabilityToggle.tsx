...
export function AdminSalesAvailabilityToggle({staff}:{staff:AdminStaff}){
 const staffId=staff.id,salesStaff=isSalesStaff(staff);
 const[receiving,setReceiving]=useState(true),[busy,setBusy]=useState(false),[message,setMessage]=useState('');
 const load=useCallback(async()=>{try{const res=await fetch('/api/admin/sales-availability',{cache:'no-store'}),json=await res.json() as ApiData&{error?:string};if(!res.ok)throw new Error(json.error||'Không đọc được trạng thái nhận khách.');const states=json.states||{};setReceiving(states[staffId]?.receivingCustomers!==false);try{localStorage.setItem(SALES_AVAILABILITY_KEY,JSON.stringify(states));window.dispatchEvent(new Event(SALES_AVAILABILITY_EVENT))}catch{}}catch(error){setMessage(error instanceof Error?error.message:'Không đọc được trạng thái nhận khách.')}},[staffId]);
 useEffect(()=>{if(salesStaff)void load()},[salesStaff,load]);
 if(!salesStaff)return null;
 async function toggle(){const next=!receiving;setBusy(true);setMessage('');try{const res=await fetch('/api/admin/sales-availability',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({receivingCustomers:next})}),json=await res.json() as ApiData&{error?:string};if(!res.ok)throw new Error(json.error||'Không thể đổi trạng thái nhận khách.');const states=json.states||{};setReceiving(states[staffId]?.receivingCustomers!==false);try{localStorage.setItem(SALES_AVAILABILITY_KEY,JSON.stringify(states));window.dispatchEvent(new Event(SALES_AVAILABILITY_EVENT))}catch{}}catch(error){setMessage(error instanceof Error?error.message:'Không thể đổi trạng thái nhận khách.')}finally{setBusy(false)}}
